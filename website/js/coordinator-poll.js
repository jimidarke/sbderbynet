...
//     readyCell.className = timer.ready ? "timer-ready" : "timer-not-ready";
//     readyCell.textContent = timer.ready ? "Ready" : "NOT READY";
...
function inject_progress_bars(control_group, round) {
  if (round.roster_size > 0) {
    ...
    control_group
      .find(".racers .bar2")
      .width((100 * passed) / round.roster_size + "%");
    if (passed > 0) {
      control_group
        .find(".racers .bar1")
        .width((100 * (round.passed - round.unscheduled)) / passed + "%");
    }
  }
  ...
  if (round.heats_scheduled > 0) {
    control_group
      .find(".heats .bar1")
      .width((100 * round.heats_run) / round.heats_scheduled + "%");
  }
}
...
function inject_progress_text(control_group, round) {
  ...
  control_group
    .find("[data-name=scheduled]")
    .text(round.passed - round.unscheduled);
  control_group
    .find("[data-name=n_heats_scheduled]")
    .text(round.heats_scheduled);
...
function generate_scheduling_control_group(round, current, timer_state) {
  ...
  if (round["next-round"]) {
    control_group
      .find("h3.roundclass")
      .append('<div class="next_tag">NEXT</div>');
  }
  ...
  control_group.on("click", function () {
    ...
    if (closed) {
      control_group
        .find("img[data-name=triangle]")
        .attr("src", "img/triangle_south.png");
  ...
  if (round.roundid == -1) {
    ...
    if (round.heats_scheduled > 0 && round.heats_run == 0) {
      if (round.adjustments && round.adjustments.length > 0) {
        buttons
          .append(
            $('<div class="late-arrival-prompt"></div>')
              .append(
                "<p>The race schedule needs to be regenerated because:</p>"
              )
    ...
    if (round.roundid != current.roundid) {
      ...
      if (round.heats_run > 0) {
        ...
        round.heats_scheduled > 0 &&
        round.heats_run < round.heats_scheduled
      ) {
        buttons.append(
          '<input type="button"' +
...
function generate_current_heat_racers(new_racers, current, nlanes) {
  ...
  $("#now-racing-group .heat_number")
    .empty()
    .append(
      "<h3>Heat " + current.heat + " of " + current.heats_scheduled + "</h3>"
    );
  var heat_lineup = $("#now-racing-group .heat-lineup")
    .empty()
    .append(
  ...
  for (var lane = 1; lane <= nlanes; ++lane) {
    var r = racers.find(function (rr) {
      return rr.lane == lane;
    });
    var hr = false;
    var nr = false;
    if (holding) {
      hr = current.heat_results.find(function (h) {
        return h.lane == lane;
      });
      nr = new_racers.find(function (n) {
        return n.lane == lane;
      });
    }
...
function offer_new_rounds(rounds, classes) {
  if (g_new_round_modal_open) {
    console.log(
      "Skipping offer_new_rounds because g_new_round_modal_open is set"
    );
    return;
  }
  ...
  for (var i = 0; i < rounds.length; ++i) {
    ...
    if (
      highest_rounds[classid]
        ? highest_rounds[classid].round < round.round
        : true
    ) {
  ...
  $("#add-new-rounds-button").toggleClass(
    "hidden",
...
function process_coordinator_poll_json(json) {
  ...
  $("#playlist-group").toggleClass(
    "hidden",
    json["current-heat"].use_master_sched
  );
  ...
  if (json["current-heat"].use_master_sched) {
    ...
    if ($("#master-schedule-group .control_group").length == 0) {
      ...
      inject_into_scheduling_control_group(
        totals,
        json["current-heat"],
        json["timer-state"]
      );
  ...
  if (matched) {
    ...
    $.each(json.rounds, function (index, round) {
      ...
      if (round.roundid == json["current-heat"].roundid) {
        generate_current_round_control_group(
          round,
          json["current-heat"],
          json["timer-state"]
        );
    ...
    $.each(json.rounds, function (index, round) {
      ...
      if (round.heats_scheduled == 0) {
        $("<div class='master_heat_reminder'></div>")
          .append(
            "<span class='master_heat_msg'>&nbsp;Not scheduled&nbsp;</span>"
          )
  ...
  generate_current_heat_racers(
    json.racers,
    json["current-heat"],
    json["timer-state"].lanes
  );
  ...
  if (json["current-heat"].roundid == -100 && json["current-heat"].now_racing) {
    ...
    $("#now-racing-group")
      .empty()
      .append(
        $("<div id='timer-testing-herald'></div>").append(
          $("<h3></h3>")
            .append($("<span></span>").text(json["refused-results"]))
            .append(
              json["refused-results"] == 1
  ...
  $("#not-racing-warning").toggleClass(
    "hidden",
    json["current-heat"]["now_racing"]
  );

  $("#playlist-start").toggleClass(
    "hidden",
    !(
