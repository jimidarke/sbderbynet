...
function toggleSimulationMode() {
  // If the fake timer window already exists and is still open, don't reopen
  if (window.fakeTimerWindow && !window.fakeTimerWindow.closed) {
      console.log('Fake timer window already open.');
  } else {
      // Open fake timer in a new popup window
      window.fakeTimerWindow = window.open(
          'fake-timer.php',
          'FakeTimerWindow',
  ...
  // Wait a bit for the fake timer window to load
  setTimeout(() => {
      $.ajax(g_action_url, {
          type: 'POST',
          data: {
          ...
          success: function () {
              $.ajax(g_action_url, {
                  ...
                  success: function () {
                      ...
                      const pollTimer = setInterval(() => {
                          try {
                              const win = window.fakeTimerWindow;

                              if (
                                  win &&
                                  win.g_auto_mode !== undefined &&
                                  win.$ &&
                                  win.initializeSimulation
                              ) {
                                  // Only auto-enable if the user hasn't already manually toggled auto mode
                                  if (!win.$('#auto-mode-checkbox').prop('checked')) {
                                      win.g_auto_mode = true;
                                      // win.$('#auto-mode-checkbox').prop('checked', true)
                                      win.initializeSimulation();
                                      console.log('Auto mode enabled programmatically');
                                  } else {
                                      console.log('Auto mode already enabled manually; use Start button to begin');
                                  }
