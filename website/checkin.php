...
<head>
  ...
  <script type="text/javascript">
    ...
    var g_preferred_urls = <?php echo json_encode(
                              preferred_urls(/*use_https=*/true),
...
<body>
  ...
  $sql = checkin_table_SELECT_FROM_sql()
    . ' ORDER BY '
    . ($order == 'car' ? 'carnumber, lastname, firstname' : ($order == 'class'  ? 'class_sort, rank_sort, lastname, firstname' : ($order == 'partition' ? 'partition_sortorder, lastname, firstname' :
          'lastname, firstname')));
...
<script>
    ...
    $(function() {
      var partitions = <?php echo json_encode(
                          $partitions,
                          JSON_NUMERIC_CHECK | JSON_UNESCAPED_SLASHES |
                            JSON_HEX_AMP | JSON_HEX_TAG | JSON_HEX_APOS
                        ); ?>;
      var partition_label_pl = <?php echo json_encode(
                                  partition_label_pl(),
                                  JSON_NUMERIC_CHECK | JSON_UNESCAPED_SLASHES |
                                    JSON_HEX_AMP | JSON_HEX_TAG | JSON_HEX_APOS
                                ); ?>;
...

      {
        ...
        $("#edit_partition").on('change', function(ev) {
          on_edit_partition_change(ev.target, reorder_modal);
        });
        $("#bulk_who").on('change', function(ev) {
          on_edit_partition_change(ev.target, reorder_modal);
        });
      }
