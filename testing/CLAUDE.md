...
- `test-adhoc-rosterless.php` — Offline functional test for the roster-less ad-hoc racing redesign (`docs/ADHOC.md`). Builds a throwaway "official" DB from the real schema and exercises the real `inc/db-marker.inc`, `inc/adhoc.inc` (`adhoc_build` is fresh-from-schema **not** a copy, and roster-less; `adhoc_age_groups`; `adhoc_arm_heat` incl. bye-lane physical-lane preservation + all-or-nothing validation), and `inc/adhoc-standings.inc` (best-single-time, DNF exclusion, per-age-group top-N, PII-free render), plus the new `RaceChart.display_pinny`/`agegroup_classid` columns, atomic marker round-trip, and official-DB isolation. Run directly: `php testing/test-adhoc-rosterless.php` (needs PHP CLI with `pdo_sqlite`; no server/Docker). Exit 0 = pass, 2 = skipped (no driver).
