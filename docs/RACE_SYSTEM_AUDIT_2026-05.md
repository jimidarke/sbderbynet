...
>
> **Fourth pass (2026-05-21, live finishtimer flash — FT001/FT002/FT003):** four *more* bugs surfaced only after the third-pass fixes let the finishtimer satellites actually reach steady state. Each blocked the next, in strict cascade. Full write-up in [`extras/imaging/README.md` § Lessons learned](../extras/imaging/README.md#lessons-learned-2026-05-bring-up) (bugs 6–9). Briefly:
> - **`wpa_supplicant@wlan0.service` reads `/etc/wpa_supplicant/wpa_supplicant-wlan0.conf`** — not `wpa_supplicant.conf`. Our customize.sh wrote to the latter path → service started, couldn't open its conf, exited, WiFi never associated. Fixed by renaming the rendered conf in both `finishtimer/` and `derbydisplay/` customize.sh and disabling the base image's auto-enabled `wpa_supplicant.service`.
> - **Build-time rsync vs `derby-pull` layout mismatch on finishtimer.** Build-time staged `extras/soapbox/infra/finishtimer/files/` → `/opt/derbynet/` (flattened); derby-pull at boot rsyncs `…/finishtimer/` → `/opt/derbynet/` (preserves `files/`). After first boot, `finishtimer.py` was at `/opt/derbynet/files/finishtimer.py` but the service unit said `/opt/derbynet/finishtimer.py`. Fixed by updating `ExecStart` to match the post-pull layout.
> - **Missing Python deps**: `tm1637` (7-seg LED driver, PyPI only as `raspberrypi-tm1637`) and `smbus2` (I²C abstraction, in Debian as `python3-smbus2`). Old `setup.sh` pip-installed them; new image pipeline didn't. Fixed in `finishtimer/customize.sh` (apt + pip).
> - **I²C bus not enabled on finishtimer images.** Missing `dtparam=i2c_arm=on` in `/boot/firmware/config.txt` → `/dev/i2c-1` absent → MCP3421 ADC read returned `None` → `getBatteryPercent()` raised `TypeError` in a daemon thread → process died. Fixed.
>
> The full sequence of bugs across third + fourth passes (1: venv-nuke → 2: MQTT_BROKER → 3: ConditionFirstBoot → 4: nologin → 5: WatchdogSec → 6: WiFi conf path → 7: ExecStart path → 8: missing Python deps → 9: I²C disabled) is the canonical "what flashing a real card teaches you that CI green can't" story. All nine documented patterns + smoke checks in the imaging README.
>
