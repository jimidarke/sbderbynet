...
- [docs/PUBLIC_STATS.md](docs/PUBLIC_STATS.md) — Spectator-facing static pages served from the cloud-twin (schedule + last 3 results), gated by an obfuscated per-event token. Run via `scripts/derbyvps.sh stats-token rotate`. Updates are event-driven (Pi push on schedule/heat/round/result changes → render-on-DB-change → version-bump browser poll, ~5–12 s), with a 30 s timer backstop; see its "Event-driven freshness" + "Testing" sections.
- [docs/PUBLIC_STATS_TODO.md](docs/PUBLIC_STATS_TODO.md) — Improvement backlog for the spectator pages (multi-pinny following, event-ended state, etc.). Ranked by user-value-per-effort. The 2026-06-15 event-driven rework already landed latency-tightening + push-not-poll (marked superseded there).
- [docs/SIMULATION_REPORT_2026-06.md](docs/SIMULATION_REPORT_2026-06.md) — Pre-race-day simulation validation: 7 elimination-engine bugs found+fixed, campaign results, std-vs-drop-slowest scoring assessment. Simulator lives in `testing/simulator/` (`simctl`).
- [docs/TESTING_DAY_2026-06-13.md](docs/TESTING_DAY_2026-06-13.md) — Session log: git divergence resolved to origin/master, pulled-code validation, Pi brought to code compliance via surgical rsync (image `e07ee621`→`6ece5c7d`), cloud-sync/VPS-access discovery, remaining host-config drift, and local PHP test-harness gotchas (CRLF scripts, `$_SERVER` env shim).
- [docs/ADHOC.md](docs/ADHOC.md) — Ad-hoc "come-as-you-are" racing mode: roster-less, coordinator-driven, isolated `adhoc.sqlite3` via active-db marker, best-single-time per-age-group scoring. Deployed to race-day Pi 2026-06-19; cloud spectator surfacing shipped (1192599a). Candidate to become the practice-day default — see its "Edge cases & hardening backlog" section.
- [docs/ADHOC_TODO.md](docs/ADHOC_TODO.md) — Ranked ad-hoc edge-case & hardening backlog (operator-error catches to land before promoting ad-hoc to the practice-day default). Captured 2026-06-21 after the Friday-practice success.
- [docs/POST_EVENT_STATS_2026-06-19.md](docs/POST_EVENT_STATS_2026-06-19.md) — Post-event snapshot (captured 2026-06-26) of the rain-cancelled St-Albert 2026: Friday-practice ad-hoc results (CSV locations + sanity), audience feedback (timer-accuracy + missing-heat flags), and partial spectator hit stats. Records the Caddy-log-rotation gotcha that lost the Friday peak, plus open follow-ups.
