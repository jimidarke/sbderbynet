...
All four had a common signature: **the failure was invisible at build time** (the CI build went green) **and only showed up on a live Pi**. The smoke-test checklist above is the gate that catches them now; run it on a real card before tagging a release.
