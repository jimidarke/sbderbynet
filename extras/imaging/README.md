...
| `/etc/resolv.conf` | static DNS (resolved not installed) — all roles | `_common/rootfs/` + symlink-strip in `_common/customize.sh` |
| `/etc/systemd/timesyncd.conf` | quiet NTP poller, IP-first (DNS-independent) — all roles | `_common/rootfs/` |
...
4d. **DNS + clock (all roles, on an internet-connected LAN):** `readlink -f /etc/resolv.conf` is a real file (NOT a `…/stub-resolv.conf` symlink) and `getent hosts pool.ntp.org` resolves; `timedatectl` reports `System clock synchronized: yes` and `timedatectl timesync-status` shows a non-zero `Packet count`. Catches the 2026-06-17 fleet bug: systemd-resolved isn't installed, so the orphaned stub symlink killed all DNS → timesyncd never synced (clock stuck at build date) on finishtimer + derbydisplay (derbypi had a role-local fix; now consolidated to `_common`). On an air-gapped LAN, sync staying `no` is expected and fine.
